...
from autobot_backend_packages import road_small, road_big, air_single, air_multiple  # Assuming your functions are in 'your_module.py'

# Function to calculate delivery time based on distance
def calculate_time(distance):
    # Assuming average speed is 5 km/h (5000 meters per hour)
    time_in_hours = distance / 5000
    time_in_minutes = time_in_hours * 60  # Convert to minutes
    return round(time_in_minutes, 2)

# Function to handle delivery type and trigger relevant function
def handle_delivery():
    ...
    # Check the delivery type and call the corresponding function
    ...
    # Calculate time for delivery
    delivery_time = calculate_time(distance)

    # Show the time and open the map
...
# Function to toggle full-screen mode with 'Esc' key
...
# Create a frame for centering widgets
frame = tk.Frame(root)
...
# For multiple end coordinates (only for "Big Route" and "Air Route (Multiple)")
