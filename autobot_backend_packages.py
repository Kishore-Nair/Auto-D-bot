...
def interpolate(start, end, interval):
    pts = [list(start)]
    dist = geodesic(start, end).meters  # Distance in meters
    if dist > interval:
...
def bellman_ford_shortest_path(graph, source, target, weight="weight"):
    ...
    for _ in range(len(graph.nodes) - 1):
        for u, v, data in graph.edges(data=True):
            edge_weight = data.get(weight, 1)  # Default weight is 1 if not provided
            if nodedistance[u] + edge_weight < nodedistance[v]:
                nodedistance[v] = nodedistance[u] + edge_weight
                nodepredecessor[v] = u
            if nodedistance[v] + edge_weight < nodedistance[u]:  # Handle undirected edges
...
def road_big(startli:list, endli:list):
    ...
    # Compute the full route based on optimal order
    route_nodes = []
    routecoords = []
...
def air_single(startli:list,endli:list):
    ...
    start = tuple(startli)  # Example start: New York (latitude, longitude)
    end = tuple(endli)  # Example end: Another point in New York
...
#
# startli = [40.768731, -73.981803]  # Example start point in Central Park
# endli = [40.771133, -73.974187]   # Example end point in Central Park
#
